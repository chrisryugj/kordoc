...
*   **🏛️ 정부 표준 공문서 생성 (v4.0)**: 실제 정부 양식 16종 + 실결재 기안문 60건을 전수 디코드·대조해 만든 공문서 엔진. 개조식 보고서(표지·목차 배너·로마숫자 장헤더·쪽번호·결재란), 기안문(별지 제1호서식 두문·결문, "끝." 자동), 공고문·보도자료 프리셋, 항목부호 8단계(1. 가. 1) 가)…) 자동, 공문서 표기법 검수 19룰(`kordoc lint`)까지 — 한글 COM 실렌더로 조판까지 실측 검증했습니다.
...
PDF 표 경계와 두 단 지면 읽기 순서를 복원했습니다. 세로선만 내려 그은 머리·합계 행, 머리행만 음영 상자로 두른 표, 가로 괘선만 있는 학술지 표를 한 표로 읽고, 두 단 지면에서는 그림 옆 캡션을 먼저, 각주를 두 단 본문 뒤에 읽습니다. ODL 공개 PDF 200문서 종합은 0.775354(4.15.4)에서 0.905513으로 올랐습니다. PDF 제목 오탐 두 유형(#89)과 없는 입력 파일의 실패 코드(`FILE_NOT_FOUND`, #88)도 바로잡았습니다. 세부 지표와 OCR 게이트 기준 재산정 사유는 [CHANGELOG](CHANGELOG.md)를 참고하세요.

## v4.15.4 변경사항

한국 PDF의 밀집 표 행과 클립 경계를 복원해 표 exact가 2,512/2,692에서 2,526/2,692로 늘었습니다. ODL 공개 PDF 200문서에서는 제목과 정렬된 수치표를 개선해 종합 점수가 0.724617에서 0.775354로 올랐습니다. 외부 하이브리드의 기존 예측을 재채점한 0.906572에는 아직 못 미칩니다. 남은 구조 오류와 세부 지표는 [CHANGELOG](CHANGELOG.md)를 참고하세요.

## v4.15.3 변경사항

쪽을 넘는 중첩표의 머리 행과 다음 쪽 본문이 감싸개 때문에 갈라지던 사례를 복원했습니다. PDF 중첩표 정답 176개 중 정확 일치는 157개에서 158개로 늘었고, 최상위 표 구조 정확 일치는 유지됐습니다. 셀 클립이 많은 PDF 쪽에서는 가까운 변끼리만 비교하도록 바꿔 클립 묶음 단계의 시간을 줄였습니다. 전체 문서 속도와 남은 정확도 한계는 [CHANGELOG](CHANGELOG.md)를 참고하세요.

## v4.15.2 변경사항

PDF 표의 좁은 시각적 간격을 빈 행·열로 잘못 늘리던 경우를 줄였습니다. 여러 행에 반복되는 실제 빈 열은 유지해, 기존 PDF 표 정답 2,692개의 정확도는 v4.15.1과 같습니다. 밀집 괘선의 교차점 처리에 쓰는 메모리도 줄였습니다. 일부 한 행짜리 머리표와 법령 양식의 빈 칸 구조는 아직 정확히 복원하지 못합니다. 자세한 수치와 남은 문제는 [CHANGELOG](CHANGELOG.md)를 참고하세요.

## v4.15.1 변경사항

PDF 표 복원과 대형 입력의 안정성을 보완했습니다.

- **PDF 안정성과 속도**: 매우 긴 괘선 때문에 메모리가 폭증하던 문제를 고치고, 쪽 처리 후 자원 해제와 반복 그림 처리를 개선했습니다. 입력 ArrayBuffer를 재사용할 수 있으며, 쪽 수 상한으로 잘린 범위는 경고합니다.
- **PDF 글·표**: 원점이 이동하거나 회전된 쪽과 Form XObject 안의 좌표 처리를 바로잡았습니다. 좁은 빈 열, 쪽을 넘는 중첩표·비고를 복원하고 표 아래 주석·출처가 꼬리말로 삭제되지 않게 했습니다. 쪽별 마크다운도 제공합니다.
- **OCR**: JPEG의 EXIF 방향을 반영합니다. 큰 이미지는 최대 24 MP로 축소하고, 검출 상자는 3,000개까지 인식하며 축소·상한 초과는 경고합니다. 괘선 검출의 반복 계산도 줄였습니다.
- **인명 마스킹**: 명시적인 속기록·회의록 문맥의 이름 4건을 추가 탐지했습니다(opt-in). 같은 외부 정답에서 361/411, 오탐 0건이며 미탐 50건은 남아 있습니다.

자세한 변경 내용은 [CHANGELOG](CHANGELOG.md)를 참고하세요.
...
읽기 품질을 전면 점검했습니다. 코퍼스를 넓히고(rhwp 표본 1,351파일·정책브리핑 hwpx+pdf 짝 200쌍) 포맷마다 정답지 벤치를 다시 세운 뒤 표·PDF·OCR·개인정보 마스킹을 고쳤고, 검색 색인처럼 글자만 필요한 호출자를 위한 두 가지를 더했습니다.

- **📊 PDF 표 복원**: 쪽을 넘는 표를 잇고, 표 안의 표를 틀 칸 안에 넣습니다. hwpx↔pdf 430쌍 1,784표 정확 일치 64.1% → 90.1%, 셀 F1 0.790 → 0.945.
- **🧾 HWP·HWP3·HWPX 표와 주석**: HWP 표를 HWPX 와 같은 규칙으로 배치해 hwp↔hwpx 쌍 3,593표가 모두 정확히 일치합니다(0.824 → 1). HWP3 표 격자를 복원하고, HWPX 각주·미주 번호와 문서 정보(제목·지은이 등)를 읽습니다.
- **👓 OCR**: 기울기를 보정하고 세로로 이어진 글 상자를 나눠 읽으며 공문서 기호(○·△·따옴표)를 되살립니다. 글자 오류율 0.201 → 0.171, 3° 기울어진 스캔 57.6% → 22.0%.
- **🕶️ 개인정보 마스킹**: 파일 안 모든 글자 저장소(머리말·각주·글상자·필드·미리보기·문서 정보 등)를 같은 길이로 가리고 결과를 다시 훑습니다. 5,081문서 오탐 387 → 0.
- **💲 본문의 `$` 가 수식이 되지 않습니다**: 한글 문서(HWPX·HWP·HWP3)의 리터럴 `$` 는 `\$` 로 내고 `$…$` 는 수식에만 씁니다. "echo $HOME $PATH" 같은 글이 마크다운 렌더러에서 수식으로 바뀌던 것. 역슬래시 뒤 구두점("C:\.Pls")에서 역슬래시가 사라지던 것도 고쳤습니다.
- **📝 미기입 누름틀 안내문을 빼고 읽습니다**: 한컴이 화면에만 흐리게 보이고 인쇄하지 않는 HWPX 안내문을 마크다운에서 뺍니다(블록 글에는 남김). 누름틀을 채우면 수정 표시를 켭니다.
- **🖼️ 이미지 끄기** (`images: false` / `--no-images`): 결과에 이미지 바이트를 싣지 않습니다. 그림 자리 표시(`![image](…)`)와 본문 글자는 그대로라 그림 위치는 알 수 있습니다. PDF 는 PNG 인코딩을 건너뜁니다. 그림 많은 PDF 실측: JSON 출력 200MB → 9.5MB, CPU 시간 30% 감소.
- **🚀 상주 파싱 워커** (`kordoc parse-worker`): 프로세스를 띄워 둔 채 stdin 요청 한 줄마다 한 줄 JSON 을 돌려줍니다. 파일마다 node 를 새로 띄우지 않아 대량 색인에 알맞습니다. 응답에 메모리 사용량(`rss`)을 실어 호스트가 워커 교체 시점을 정할 수 있습니다.

## v4.14.2 변경사항

공문서 출력을 한글 2024 실렌더 PDF로 다시 대조해 "진짜 공문처럼" 다듬었습니다.

- **📐 항목 둘째 줄이 첫 줄 내용과 정확히 맞습니다**: 부호(□ ○ - ※ 1. 가.) 뒤를 공백이 아니라 탭으로 두고, 한글의 "내어쓰기용 자동 탭"을 씁니다. 양쪽 정렬이 부호 뒤 공백을 늘려 첫 줄 내용이 밀리던 것, 자간을 줄인 줄에서 내용이 당겨지던 것이 구조적으로 없어졌습니다. 기안문·보고서·계획서·통지·회의록·업무보고·개조식·보도자료 전부.
- **✂️ 낱말이 줄 끝에서 쪼개지지 않습니다**: 본문을 어절 단위 줄바꿈으로 바꾸고("동대/문" → "동대문") 외톨이줄 보호를 켰습니다. 날짜·금액은 묶음 빈칸으로 묶어 "2026. 1. / 22." 처럼 갈라지지 않습니다. 곧은따옴표는 ‘ ’ “ ”로 바꿉니다.
- **📏 한글 글꼴 실폭으로 조판을 재현합니다**: 한컴돋움·HY견고딕·휴먼명조 등 8종의 실제 글자 폭으로 줄바꿈을 계산합니다(한글 2024 PDF 154줄 전부 일치). 마지막 줄이 짧으면(줄 폭 절반 이하) 자간·장평을 최대 15%까지 줄여 한 줄 올리고, 한 줄에 맞춘다는 판단엔 2% 여유를 둬 한글에서 넘치지 않게 했습니다. 공백 없는 긴 목록("구(강남·강서·…·종로)가")처럼 한 줄보다 긴 어절이 있는 문단은 그 문단만 글자 단위로 조판하되 가운뎃점·쉼표 뒤에서만 끊어, 앞 줄이 크게 벌어지지 않습니다.
- **🧩 PPTX 감지** (#80, PR #81 @LimePencil): PPTX 를 넣으면 "HWPX 섹션 없음" 대신 지원하지 않는 형식이라고 알려 줍니다.
- **🧹 기술부채**: 1,200줄 넘던 `mcp.ts`·`cli.ts`·`hwp5/parser.ts`·`svg-render.ts` 를 기능별 모듈로 나눴고, 파서가 묶음 빈칸을 버리던 것과 `kordoc render` 미리보기의 탭 폭·줄 끝 공백 정렬을 고쳤습니다.
...
- **📐 위계 = 실결재 그대로**: 개조식형 □ HY견고딕 17pt 굵게(0타) → ㅇ 한컴돋움 15pt 굵게(1타) → - 휴먼명조 14pt(3타) → ㆍ, ※ 한컴돋움 14. 법정형 굴림체 12pt 160%, `1.` 0타 → `가.` 2타 → `1)` 4타(2타 계단), 내어쓰기는 부호폭+1타. 표는 한컴돋움 12(헤더 굵게·#DFE6F7). 근거는 `docs/gongmunseo-reference.md` 2.8.
- **📏 □·제목은 절대 두 줄로 안 꺾입니다**: 장평 → 자간 → 글자 크기 순으로 줄여 한 줄에 담고(실측 관행), 그래도 안 되면 경고합니다. 표 셀도 12→11→10pt 자동 축소.
- **📋 기안문 두문표·결문표**: 실결재 표 그대로 — 슬로건·기관명(굴림 20 굵게 자간띄움)·수신/(경유)/제목, 발신명의·회색 띠·결재선(직위/성명/일자)·협조자·시행/접수·주소·전화/전송/이메일/공개구분. 수신이 '내부결재'면 발신명의 생략. `--doc-head "org=광진구,slogan=…,to=…,title=…"` `--doc-foot "sender=…,drafter=주무관 홍길동,…,zip=…"`.
- **📑 보고서(서울 실무 보고서형)**: 제목표(HY헤드라인M 25 굵게, 상 0.4mm/하 0.15mm) + 담당자 행(`--report-info`) + 요약박스(`--summary` 또는 제목 직후 `>`) + `Ⅰ. Ⅱ.` 장(h2) + □/ㅇ/- 본문. `--cover --dept 스마트도시과 --doc-info "docNum=…"` 로 표지(문서정보표·결재선·파랑 띠 제목·기관명).
- **✍️ 실무 디테일**: `출처:`/`자료:`/`근거:` 줄은 자동으로 ※ 참고(13pt 작은 글씨), 둘째 줄이 한 줄의 20% 이내로 삐져나온 항목은 자간을 줄여(Shift+Alt+N 관행) 한 줄에 담고, 줄바꿈은 실결재 76%가 쓰는 글자 단위+양쪽정렬(어절 유지는 긴 어절 앞 줄이 벌어져 기각, 영문·숫자는 단어 유지), 법령 코드 `(282791)`·KOSIS 표 ID `DT_…`·"○○ MCP 조회" 같은 내부 식별자는 자동 제거. 보고서는 제목 직후 인용문(`>`) 한 문장("…하고자 함", 부호 없이 3줄 이내)이 요약박스가 됩니다(없거나 넘치면 경고).
- **⚙️ 기본값 변경**: 기안문 본문 굴림체 12(함초롬바탕은 `--font myeongjo`), 보고서·계획서 여백 13/13/18/18·줄간격 180%·2단계 부호 ㅇ(중앙부처 개조식만 ○), h2는 장(`--h2-marker roman|number|box|none`). 개조식(`개조식` 프리셋)·보도자료는 종전 그대로.
- **🏷️ 장 제목 띠 표**: 보고서·계획서의 `## 장 제목`이 로마자 채움 칸 + 제목 칸(밑줄) 표로 나옵니다(계획서 실결재 37~39% 관행). 종전 `Ⅰ. 제목` 텍스트는 `--h2-marker roman`. 맥 한컴에서 글꼴 칸이 비고 스타일이 영문명으로 뜨던 산출물 결함도 수정(7개 언어 글꼴 슬롯 통일·`langID`).
- **📊 표 열폭은 중요도 순**: `비고`·`근거`·`참고`·`담당`·`연락처` 열과 내용 열이 있는 표의 마지막 열은 25% 이내로 좁게(실결재 462표에서 비고 열 중앙값 21%), `내용`·`사항`·`실적`·`계획`류 열은 넓게(중앙값 51%) 왼쪽 정렬. 긴 근거 텍스트가 최광열을 차지하고 내용이 두 줄로 꺾이던 결함. 요약박스는 문단 좌우 여백 1000(실측 55%)으로 글자가 테두리에 붙지 않습니다.
- **📊 코퍼스**: `bench/corpus-gen/opengov-2609/` 7종 필터 280건 추가(계획·결과보고·방침·안내·회의·2024·2022).
- **💰 금액 한글 병기 자동 계산**: `금113,560원` 처럼 한글 병기가 빠진 금액은 경고와 함께 `금113,560원(금일십일만삼천오백육십원)` 실제 값을 제안합니다(규정 시행규칙 제2조). 하이픈 날짜 `2026-07-18` 도 잡아서 `2026. 7. 18.` 로 바꿔 줍니다(URL·파일명·표 셀은 제외).
- **🔣 항목 부호 확장**: `❑ ❏ ㅁ`(대항목)·`◎`(중항목)·`ㅡ ‣ ▪ ▫`(세부) 도 □/ㅇ/- 와 같은 위계로 읽습니다.
- **🖼️ 렌더 통합 계층 (#75)**: `renderDocument(파일, { format: "svg"|"html"|"png"|"jpeg"|"pdf", pages: "2-4" })` 하나로 페이지별 SVG·자급자족 레이아웃 HTML·페이지별 PNG/JPEG·PDF(Chromium 필요)를 냅니다. 렌더 결과에는 문단·표·이미지·도형의 **페이지 로컬 pt bbox 와 결정적 id**(`RenderScene.regions`, SVG `data-kordoc-id`)가 붙고, `extractRenderedRegions()` / `kordoc crop 문서.hwpx --target table -d ./regions` 로 그 영역을 실배율로 잘라냅니다. `kordoc render --format png --pages 2-4 -d ./pages`, MCP `render_document` 의 `pages`, `crop_regions`. HWP5 렌더는 후속.
- **🧭 표 분류·시각 추출 (#76)**: `parse(파일, { classifyTables: true })` 로 표마다 `semantic-table`(데이터표) / `non-tabular-layout`(표를 캔버스로 쓴 조직도·연락망) / `uncertain` 분류가 붙습니다(휴리스틱, 네트워크·LLM 없음, 중첩표 독립 분류). `extractTables(파일)` / `kordoc tables 문서.hwpx --visual non-tabular-and-uncertain -d ./tables` 는 분류 + 렌더 영역 + crop 을 한 번에 — 조직도는 이미지로, 데이터표는 구조로 넘기는 멀티모달 파이프라인용. 병합 없는 단순 표의 셀 이미지가 마크다운에서 사라지던 결함도 함께 수정.

## v4.12.3 변경사항

공문서 작성 쪽 라운드. 실결재 기안문 206건·보고서 337건의 항목부호 단계별 글꼴을 실측해 **위계 폰트 옵션**을 넣고, 표기법 검수의 쌍점·날짜 룰 오탐을 법정 서식 근거로 좁혔습니다. 별표 코퍼스 3차(183쌍)를 더해 게이트 모수는 hwpx ~1,390·pdf ~950·hwp쌍 ~860 입니다.

- **🔤 위계 폰트 `levels`**: 항목부호 단계(0~7)마다 글꼴·크기·굵기를 지정합니다 — `--levels "0=HY견고딕/17/bold,1=한컴돋움/15/bold,2=휴먼명조/14"`, MCP `levels`. 실측: 법정 8단계(1. 가. 1))는 본문과 동일이 90% 라 기본값은 그대로이고, 전자결재 기안문의 □/ㅇ/- 계열은 □ HY견고딕 +2~3pt 굵게 · ㅇ 한컴돋움 굵게 · - 휴먼명조가 지배 관행입니다(`docs/gongmunseo-reference.md` 2.7). 내어쓰기 폭도 그 단계 글자 크기로 맞춥니다. 미지정이면 산출물 불변.
- **📝 표기법 룰 좁힘**: `COLON_SPACE` 는 표 셀을 건너뜁니다 — 법정 별지서식 자체가 "성 명 :" 라벨 셀을 쓰므로(서식 595건의 표 셀 801건 전부 라벨) 규칙 대상이 아닙니다. 본문의 "일 시 : 값"(기안문 206건 중 100건, 414건)은 편람상 위반이라 그대로 경고합니다. HTML 태그·마크다운 강조 닫힘 뒤 쌍점("성명:</td>", "과정:** 붙임") 오탐도 제거. `DATE_NO_SPACE` 는 법제처 연혁 표기(`<개정 2012.2.14>`·`삭제 <2016.2.29.>`·`[시행일:2017.9.8.]`)를 제외합니다(서식 156건 → 10건). 기안문의 문서번호 인용 "○○과-123(2026.7.1.)호"(103건)는 편람 날짜 표기 위반이라 유지.
- **📄 PDF 가운뎃점**: 한컴 PDF 가 가운뎃점(ㆍ)을 조합형 아래아(U+119E)로 내던 것을 되돌립니다(별지서식 573쌍 실측 340자·41문서).
- **📄 PDF 1×1 표 줄 보존**: 선서문 안쪽 상자처럼 중첩표 없는 1×1 표는 줄마다 문단으로 내어 "선 서 나는 헌법을 …" 처럼 제목 줄이 본문에 붙지 않습니다(표 셀 줄바꿈 보존 정책과 정합).
- **📄 PDF 제목 아래 틀**: 테두리 없는 1칸 틀(별표 1×1 프레임·선서문 바깥 틀)을 "페이지 반복 클립" 으로 가르려던 가설은 반증됐고(본문 영역 클립은 쪽마다 좌표가 다르고 별표는 1쪽), 대신 **제목 아래(윗변 ≥ 20%)·본문 폭(≥ 60%)·위에 글이 있는** 컨테이너를 틀로 봅니다. 채용공고 2단 상자는 폭 조건으로 제외(pdf-table-gt 게이트 유지).
- **🖋️ HWP 결재 기호 하나 더**: 한컴 PDF 실렌더로 확인한 네모 안 "인"(U+F00E1, HWP5 A0E1)을 "(인)" 으로 냅니다. HWP5 하이픈 제어문자(0x18)는 한컴이 그리지 않으므로("60g/㎡") 더 이상 "-" 를 내지 않습니다.
- **📊 별표 코퍼스 3차**: `licbyl-byl2/` 183쌍(seed 20260907). 하위 30건 분해 — 1×1 틀 PDF 미감지 8 · HWP 레이아웃 해체 vs PDF 1열 유지 15 · 구조 차 7(수식 셀·쪽 경계 셀 분할·줄바꿈, 파서 결함 없음). HWP↔PDF 셀 일치 별표 0.728 → 0.746, 3차 0.772 → 0.787, 서식 0.965 유지.


## v4.12.2 변경사항

별지서식 라운드 2. 코퍼스를 서식 279쌍·별표 90쌍 더 모아(총 1,995파일) HWP·HWPX·PDF 세 파서가 **같은 IR 모양**을 내도록 맞췄습니다. HWP↔PDF 셀 텍스트 일치 0.927 → 0.965, PDF 표 GT cellExact 0.945 → 0.980 을 실측했고 게이트 플로어를 그 수치로 올렸습니다.

- **🪆 PDF 틀 안 중첩표**: 지정서·영치증·선서문처럼 1칸 틀 안에 "발신명의 | 직인" 표가 든 서식에서, PDF 가 그 표를 틀 뒤 별도 블록으로 흘리던 것을 틀 셀 안에 문단·표 원문 순서로 넣습니다(HWPX 파서와 같은 모양). 네 변에 테두리가 그려진 1칸 틀은 표로, 그 안에 홀로 든 상자는 중첩표로 살립니다. 테두리 없는 바깥 틀은 한컴 본문 영역 클립과 구분할 수 없어 표로 삼지 않습니다.
- **🖋️ HWP 결재란 "(인)" 복원**: HWP 5.x 는 2바이트 문자라 한컴 전용 기호(U+F012B)를 U+A000대에 접어 저장합니다. `ꄫ` 로 새던 결재란 기호가 "(인)" 으로 나옵니다.
- **📋 HWP 서식 틀 보존**: 중첩표를 품은 별지서식 3×1 틀(제목행·틀·꼬리행)을 레이아웃 표로 보고 문단으로 해체하던 것을 표로 남깁니다. 페이지 사슬 레이아웃 표(글이 많은)는 종전대로 해체합니다.
- **📝 표기법 검수 정밀화**: 실결재 기안문 206건·별지서식 595건으로 오탐을 실측해 `TILDE_SPACE` 는 숫자·날짜·시각 범위에만, `DUEUM_ERROR` 는 서식 기입란("    년도", "( 년간)")과 표 머리글을 건너뜁니다.
- **🧾 양식 인식이 틀 안 중첩표까지**: `extractFormFields` 가 셀 안 중첩표의 라벨-값을 훑습니다(`fillHwpx` 와 같은 경계).
- **📊 `bench/cmp-hwp-pdf.mjs`**: HWP↔PDF 셀 텍스트 Jaccard 보고 지표 정식 편입. `--linebreaks` 로 PDF 셀 줄바꿈 자리의 어절 경계 비율(85%·글자 단위 13%)을 실측해 줄 결합은 하지 않기로 했습니다.

## v4.12.1 변경사항

법령 **별지서식**(법제처 별표·서식) 파싱을 HWP·HWPX·PDF 세 포맷에서 맞췄습니다. 법제처 OpenAPI로 서식 300건의 HWP 원본과 PDF를 쌍으로 모으고 rhwp 변환 HWPX를 붙인 900파일 코퍼스를 게이트에 편입해, HWP↔PDF 셀 텍스트 일치 0.79 → 0.92, PDF 표 GT cellExact 0.73 → 0.96 을 실측했습니다.

- **📐 PDF 셀 클립 그리드**: 한컴 PDF는 표 셀마다 클립 사각형을 깔고 글을 찍습니다. 별지서식 외곽 표는 테두리 "없음" 셀이라 괘선만 보는 종전 경로가 표를 놓치고 제목·기입란이 헤딩과 문단으로 흩어졌는데, 이제 클립 사각형 묶음을 **셀 기하가 확정된 표**로 바로 씁니다. 도선사면허 신청서가 HWP 파서와 같은 23열·같은 병합 구조로 나옵니다. 클립의 포함 관계로 층을 나눠, 지정서·증서의 1칸 테두리 틀은 제목행·꼬리행과 한 표의 셀이 되고 안쪽 "발신명의 | 직인" 표는 별도 표로 살립니다.
- **➔ 심볼 폰트 글리프 복원**: 처리절차 화살표·체크박스를 Wingdings로 찍은 PDF에서 `è` 같은 깨진 라틴 문자가 남던 것을 유니코드(➔ □ ✔ ☑)로 되돌립니다.
- **📝 표기법 검수 6룰 보강**: 금액 괄호 한글 병기(`MONEY_NO_HANGUL`), 물결표 앞뒤 붙여쓰기(`TILDE_SPACE`), 두음법칙 '년도→연도'(`DUEUM_ERROR`), 외래어 오기 40종(`LOANWORD_ERROR`, 표준 표기 제안), 차별·비하 표현 25종(`DISCRIMINATORY_TERM`, 순화어 제안), 붙임 있는데 "끝." 없음(`END_MARK_MISSING`, `kordoc lint` 전용). 시중 HWPX 공문 도구(pyhwpxlib·hwpx-skill)의 검사 항목과 대조해 비어 있던 축을 채웠습니다.
- **🏛️ 내부결재 문서의 발신명의 생략**: 수신이 "내부결재"인 기안문은 결문에 발신명의를 적지 않습니다(행정업무규정 시행규칙).
- **🩹 기입 빈칸 보존**: "년   월   일"·"시   분"이 "년월일"로 붙던 것, 청구서류 1열 틀 표의 기입 항목이 한 줄로 뭉개지던 것을 고쳤습니다.
...
- **📝 공문서 표기법 검수 19룰**: 날짜·시간·금액·붙임·외래어·차별 표현 등을 검수하는 `kordoc lint <file>` + 공문서 생성 시 경고 병기.
...
- **🖼️ 레이아웃 보존 렌더**: `kordoc render 문서.hwpx -o 문서.svg` / `renderHwpxToSvg(buffer)` (v4.13.0: `renderDocument` 로 HTML·PNG·JPEG·PDF·페이지 선택·영역 crop) — 한컴이 저장한 조판 캐시(줄 좌표·셀 그리드·개체 앵커)를 SVG 절대배치로 그려 원본 레이아웃을 재현합니다. run별 글자 크기/굵기/색/장평/자간, 문단 정렬, 셀 배경·테두리, 병합 셀, 인라인 개체, 이미지 크롭까지. 한컴 저장본 전용(1페이지).
...
### 글자만 필요할 때 (이미지 끄기·상주 파싱 워커)

```typescript
const result = await parse(buffer, { images: false }) // 이미지 바이트 없이 (그림 자리 표시는 남김)
```

CLI 는 `kordoc 문서.pdf --format json --no-images`. 대량 변환은 `kordoc parse-worker` 를 띄워 두고
...
`ocr` 은 `"off"`·`"auto"`(OCR 필요 페이지만)·`"force"`, 그 밖에 `formulaOcr`·`password` 를 받습니다.
...
> (공문서 생성 시에는 `generate` 가 같은 룰의 경고를 함께 냅니다. `END_MARK_MISSING` 은 완성 원고를 보는 `lint` 에서만 켜집니다.)
