...
const RULES: LintRule[] = [
  ...
  // 물결표 앞뒤 붙여쓰기 — 기간·범위는 "2. 20.∼2. 24." 처럼 붙여 씀
  { code: "TILDE_SPACE", severity: "warning", pattern: /\S[ \t]+[∼~～]|[∼~～][ \t]+\S/g,
    message: "물결표(∼) 앞뒤는 붙여 씀", suggest: "예) 2. 20.∼2. 24., 09:00∼18:00" },
  // 두음법칙 — 어두의 "년도·년간·년말…" 은 "연도·연간·연말" (숫자 뒤 '2026년도' 는 정상)
  { code: "DUEUM_ERROR", severity: "warning", pattern: /(?<![가-힣\d])년(?:도별|도|간|말|초|차|세|내)(?![가-힣])/g,
