...
describe("#52 — IRCell.text 평탄화가 blocks 문서 순서를 따른다 (실문서 단일 run)", () => {
  it("셀 한 run에 [표] 텍스트 [표] 텍스트 — text가 blocks 순서로 평탄화된다", async () => {
    ...
    // 하위 호환 text = blocks의 평탄화 (문서 순서). 글자취급(treatAsChar=1) 표는 원문에서
    // 앞뒤 텍스트와 같은 줄 → 경계를 \n이 아닌 공백으로 이어 원문 한 줄을 재현한다 (#52 후속).
    // (종전 4.2.4는 "0000-00-00\n부터\n0000-00-00\n까지"로 시각충실도 회귀)
    assert.equal(cell.text, "0000-00-00 부터 0000-00-00 까지")
  })

  it("글자취급 표–텍스트–글자취급 표가 한 줄이면 평탄화도 한 줄(공백) — \\n 분리 금지 (#52 후속)", async () => {
    // 제보자 ▦∼▦ 형: 글자취급 표들이 원문 화면에서 앞뒤 텍스트와 같은 줄에 놓인다
    const cellPara = `<hp:p id="2" paraPrIDRef="0"><hp:run charPrIDRef="0">` +
      inlineTbl(210, "홍길동") + `<hp:t> ~ </hp:t>` +
      inlineTbl(211, "김철수") + `<hp:t> 연락처 :</hp:t>` + `</hp:run></hp:p>`
    const body = `<hp:p id="1" paraPrIDRef="0"><hp:run charPrIDRef="0">` +
      outerTbl(100, cellPara) + `</hp:run></hp:p>`
    const r = await parseHwpxDocument(await makeHwpx(sec(body)))
    const cell = r.blocks.find((b) => b.type === "table")!.table!.cells[0][0]
    // 순서는 문서 순서 유지, 경계는 공백 — 한 줄(줄바꿈 없음)
