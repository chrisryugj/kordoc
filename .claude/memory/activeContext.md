...
**마지막 업데이트**: 2026-07-03 (연속 세션 10: pdf표GT 매칭 100% 공략 → v3.12.0 릴리스)
**상태**: 테스트 683/683. `npm run bench:gate` 5체인 전부 PASS. tsc 13(기존 — 신규 0)

## 이번 세션 완료 (2026-07-03 연속 10차)
...
## 지표 대시보드 (2026-07-03 연속 10차 종료 — v3.12.0)

| 트랙 | 지표 | 값 | 게이트 | 비고 |
|---|---|---|---|---|
| hwpx(85) | recallMicro / phantom | **1.0** / 0.000054 | 0.999 / 0.005 | |
| hwpx | 표 exact / cellF1 | **611/611** / 1.0 | 0.99 / 0.999 | |
| pdf(48) | coverage(micro) | **0.99609** | 0.985 | 9차 0.99608 → +1e-5 (개선 7·후퇴 changwon −2e-5뿐) |
...
- **pdf표GT 매칭 트랙은 사실상 종결** (98.55%, 잔여 1 원리적 수용). 2차 지표
  (exact 65%·F1 0.72)의 남은 갭은 대부분 병합 열 표현 차·동의서류 평탄화 —
  GT 헤더의 "잔여 미달 성격" 참조 (원리적 표현 차로 분류됨)
- ice-geomjeong 라벨 탭 병리 (9차 백로그): 짧은 수직선(66.8/301.8)이 열 경계가
  되어 프로즈 찢김 — 열 지지도(support) 필터 검토. buildTableGrids 광역 영향이라
  전 쌍+coverage 파일별 대조 필수
